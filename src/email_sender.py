...
def send_email_with_reports(subject, recipient_email, excel_path, map_path, city=None):
    ...
    <html>
        <body>
            ...
            <p>Excel-отчет с детальной информацией и интерактивная карта приложены к этому письму.</p>
            <p>Для просмотра карты, пожалуйста, скачайте приложенный файл <b>{Path(map_path).name if map_path else "карта"}</b> и откройте его в вашем веб-браузере.</p>
            <br>
